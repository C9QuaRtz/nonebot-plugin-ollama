...
class ScopedConfig(BaseModel):
    model: str = '' # 标准模型
    model_img: str = '' # 仅用于响应图片请求的视觉模型
    url: str = '' # 默认可填 http://127.0.0.1:11434/
    min_priority: int = 5 # Nonebot 事件优先级
    max_histories: int = 100 # 某方对话的最大消息记忆数
    listening_group: str = [] # 希望提供服务的群聊号
    listening_private: str = [] # 希望提供服务的私聊对象QQ号
    cmd: str = [] # 触发消息处理的消息前缀
    cmd_img: str = [] # 改用视觉LLM处理的消息前缀
